...
void clear_framebuffer(uint8_t r, uint8_t g, uint8_t b)
{
	...
	uint32_t color = (r << framebuffer->red_mask_shift) | (g << framebuffer->green_mask_shift) | (b << framebuffer->blue_mask_shift);
	mempattern4(color, framebuffer->address, framebuffer->width * framebuffer->height);
}

void blit_color_rect(uint8_t r, uint8_t g, uint8_t b, uint32_t width, uint32_t height, uint32_t x, uint32_t y)
{
	struct limine_framebuffer *framebuffer = fb_request.response->framebuffers[0];
	uint32_t color = (r << framebuffer->red_mask_shift) | (g << framebuffer->green_mask_shift) | (b << framebuffer->blue_mask_shift);
	for (uint32_t h = 0; h < height; h++)
	{
		mempattern4(color, framebuffer->address + ((y + h) * framebuffer->pitch) + (x * framebuffer->bpp / 8), width);
	}
}
